...
- `interface`: Optional network interface name (for example `eth0`). When
  ...
  automatically.
  The monitor installs a kernel-space filter so that only UDP packets for the
  watched port are inspected, keeping CPU usage low even on busy links. If the
  process lacks the privilege to load the filter, it falls back to user-space
  filtering so playback from external senders continues to work (at the cost of
  some additional wake-ups on busy interfaces).
